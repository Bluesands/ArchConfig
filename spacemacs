...
`dotspacemacs/user-config' first."
  ...
  ;; Add node path
  (setenv "PATH" (concat (getenv "PATH") ":/opt/node-v6.10.0-linux-x64/bin"))
  (setq exec-path (append exec-path '("/opt/node-v6.10.0-linux-x64/bin")))
...
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(org-table ((t (:foreground "#6c71c4" :family "Source Code Pro 12")))))
